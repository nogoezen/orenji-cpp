...
void World::initialize()
{
    ...
    try
    {
        auto cityData = GameData::getInstance().loadJsonFile("data/cities.json");

        if (cityData.is_array())
        {
            for (const auto &city : cityData)
            {
                int id = city["id"];
                std::string name = city["name"];
                std::string description = city["description"];
                std::string region = city["region"];
                int posX = city["posX"];
                int posY = city["posY"];
                int population = city.value("population", 1000);

                // Parse market factors
                std::map<int, float> marketFactors;
                if (city.contains("marketFactors") && city["marketFactors"].is_object())
                {
                    for (auto it = city["marketFactors"].begin(); it != city["marketFactors"].end(); ++it)
                    {
                        int goodId = std::stoi(it.key());
                        float factor = it.value();
                        marketFactors[goodId] = factor;
                    }
                }

                m_cities.push_back(City(id, name, description, region, posX, posY, marketFactors, population));
            }

            std::cout << "Loaded " << m_cities.size() << " cities." << std::endl;
        }
    }
    catch (const std::exception &e)
    {
        std::cerr << "Error loading city data: " << e.what() << std::endl;
        // Fall back to hardcoded cities
        createCities();
    }

    // Initialize terrain after loading cities
    initializeTerrain();
}
