...
struct City
{
    ...
    City() : id(0), posX(0), posY(0), population(0) {}

    // Constructeur avec tous les paramètres
    City(int _id, const std::string &_name, const std::string &_desc, const std::string &_region,
         int _posX, int _posY, const std::map<int, float> &_factors, int _population = 1000)
        : id(_id), name(_name), description(_desc), region(_region),
          posX(_posX), posY(_posY), marketFactors(_factors), population(_population) {}
};
...
public:
    ...
    // Initialize the world data - load from files, create cities, etc.
    void initialize();
    ...
    // Find a city by ID and return a pointer to it (alias for getCityById)
    const City *findCityById(int id) const
    {
